...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/29 12:31:59 by seemil            #+#    #+#             */
/*   Updated: 2024/09/29 13:26:55 by eyetis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/minishell.h"

int	handle_red_input(t_token *token, t_cmd *cmd, bool has_last_heredoc,
		t_state *state)
{
	t_token	*temp;
	...
	if (access(temp->data, F_OK) == -1)
		return (print_execute_err(state, token->next, 1,
